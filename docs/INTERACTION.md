...
- Foreground worker collection uses a fixed 60-second deadline, configurable
  with `TACHYON_WORKER_RESULT_TIMEOUT_SECS`; progress events do not extend it.
- A worker timeout is returned as partial evidence so completed sibling results
  can still be synthesized.
...
- Tachyond remains responsible for process lifecycle and future cancellation.
